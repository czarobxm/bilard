...
from .balls_generators import *

from random import random, randint


def close():  # CLOSING PYGAME
    pygame.quit()
...
def friction_force(ball):  # SLOWING THE BALLS DOWN
...
def stopped():  # CHECKING IF BALLS ARE MOVING | FALSE IF BALLs ARE MOVING
    for ball in all_balls:
...
class Pygame2D():
    def __init__(self):  # DEFINING PYGAME, PYGAME SCREEN, PYMUNK SPACE, PYGAME CLOCK
        # HANDLING PYGAME
        self.show_game = True  # if True pygame is initialized and billiard show up on screen
        if self.show_game:
            pygame.init()
            self.pygame_screen = pygame.display.set_mode(SCREEN_SIZE)
            self.clock = pygame.time.Clock()  # CHECK IF IT DOES SOMETHING/ PROBABLY DOES
    ...
    # CHECK WHAT CLOCK DO/ WHAT IS DT/ MUST BE REFACTORED !!!!!
    def refresh_screen(self):
        self.dt = self.clock.tick(FRAMERATE)  # * FRAMERATE / 1000
        pygame.display.flip()

    # ADDING OBJECTS (BALLS, HOLES, TABLE) TO PYMUNK SPACE
    ...
    # AT THE END OF THE GAME DELETE OBCJETS
...
def _mouse_pos(self):  # GETTIN MOUSE POSITION
